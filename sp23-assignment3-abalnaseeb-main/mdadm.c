...
int mdadm_write(uint32_t addr, uint32_t len, const uint8_t *buf)
{
  ...
  while (length > 0)
  {
    ...
    jbod_operation(construct_block_operation(jbod.current_block_id, 0, jbod.current_disk_id, JBOD_READ_BLOCK), localBuff);
    seek(jbod.current_block_id, jbod.current_disk_id);
    uint32_t bytes_to_copy = (jbod.block_pointer < JBOD_BLOCK_SIZE) ? ((length < (JBOD_BLOCK_SIZE - jbod.block_pointer)) ? length : (JBOD_BLOCK_SIZE - jbod.block_pointer)) : 0;
    if (bytes_to_copy > 0)
    {
      memcpy(localBuff + jbod.block_pointer, buf + (len - length), bytes_to_copy);
      jbod_operation(construct_block_operation(jbod.current_block_id, 0, jbod.current_disk_id, JBOD_WRITE_BLOCK), localBuff);
      length -= bytes_to_copy;
      jbod.block_pointer += bytes_to_copy;
    }
    if (jbod.block_pointer >= JBOD_BLOCK_SIZE)
    {
      jbod.block_pointer = 0;
      jbod.current_block_id = (jbod.current_block_id < JBOD_NUM_BLOCKS_PER_DISK - 1) ? (jbod.current_block_id + 1) : 0;
      jbod.current_disk_id += (jbod.current_block_id == 0);
    }
  }
  uint32_t write_op = construct_block_operation(jbod.current_block_id, 0, jbod.current_disk_id, JBOD_WRITE_BLOCK);
  jbod_operation(write_op, localBuff);

  switch (jbod.current_block_id)
  {
  case JBOD_NUM_BLOCKS_PER_DISK - 1:
    jbod.current_disk_id++;
  case 0:
    jbod.current_block_id = 0;
    break;
  default:
    jbod.current_block_id++;
    break;
  }
